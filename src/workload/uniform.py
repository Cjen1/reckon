...
from reckon_types import read, write, Client, WorkloadOperation, AbstractWorkload
...
class UniformOpsProvider(AbstractWorkload):
    ...
    def prerequisites(self) -> List[WorkloadOperation]:
        return [
                (self._clients[0], write(self._new_key(k), "", 0))
    ...
    def workload(self, start) -> Iterator[WorkloadOperation]:
        ...
        while True:
            if self._should_gen_write_op():
                yield (self._clients[i % len(self._clients)], write(self._rand_key(), self._uniform_payload(), time))
            else:
                yield (self._clients[i % len(self._clients)], read(self._rand_key(), time))
