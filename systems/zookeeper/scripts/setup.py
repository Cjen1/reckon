...
def start_servers(hosts, ips, zoo_src, cgrps):
    ...
    for k, host in enumerate(hosts):
        ...
        cmd = "screen -d -m -S zookeeper_{name} bash {zkd}{bn} start-foreground {zkd}conf/zoo{k}.cfg".format(
            k=k, bn="bin/zkServer.sh", zkd=zkd, name=host.name
        )
        host.popen(cmd, preexec_fn=lambda: contain_in_cgroup(cgrps[host]))
        restarters.append(
            lambda: host.popen(cmd, preexec_fn=lambda: contain_in_cgroup(cgrps[host]))
        )
    return restarters
    ...
    cluster_infos = [
        (
            [
                ...
                + (ips[i] if (not ("i == k" == True)) else "0.0.0.0")
                + ":2888:3888"
...
def orig_install(res_cons_dir, zoo_dest):
    commands = list(
        map(
            lambda s: s.split(" "),
            """wget http://apache.mirror.anlx.net/zookeeper/stable/apache-zookeeper-3.5.6-bin.tar.gz -O {zd}/apache-zookeeper-3.5.6-bin.tar.gz
mv {zd}/apache-zookeeper-3.5.6-bin/* {zd}
    ...
    while "failure" in check_output(commands[0]):
        print("wget failed...")
    tf.open(
        "{zd}/apache-zookeeper-3.5.6-bin.tar.gz".format(zd=zoo_dest), "r:gz"
    ).extractall(zoo_dest)
    ...
    for f in os.listdir("{zd}/apache-zookeeper-3.5.6-bin/".format(zd=zoo_dest)):
        ...
        os.rename(
            "{zd}/apache-zookeeper-3.5.6-bin/{f}".format(zd=zoo_dest, f=f),
            "{zd}/{f}".format(zd=zoo_dest, f=f),
        )
    os.mkdir("{zd}/data".format(zd=zoo_dest))
    print(
        "cp {rc}/systems/zookeeper/zkconfs/zoo_local.cfg {zd}/conf/zoo.cfg".format(
            rc=res_cons_dir, zd=zoo_dest
        )
    )
    copyfile(
