...
def read_payload(pipe):
    ...
    if payload:
        ...
        print("Got nothing from pipe, probably EOF")
        return None
...
def ready(clients):
    ...
    print("Waiting to receive readys")
    for client in clients:
        recv_one()
...
def execute(clients, failures, duration):
    ...
    for failure_time, failure in zip(failure_times, failures):
        ...
        print("BENCHMARK: sleeping for" + str(sleep_time))
        sys.stdout.flush()
        if sleep_time > 0:
            time.sleep(sleep_time)
        failure()

def collate(pipes, config):
    print("COLLATE")
    resps = []
    inputs = pipes
    while inputs:
    ...
    print("COLLATE: done, writing: " + config['f_dest'])
    with open(config['f_dest'], "w") as fres:
        json.dump(resps, fres)

    print("COLLATE: written to file")
...
def run_test(f_dest, clients, ops, rate, duration, service_name, client_name, ips, failures):
    ...
    t_collate = Thread(
            target=collate,
            args=[[out_pipe for (in_pipe, out_pipe) in microclients], config]
            )
    ...
    print("Waiting to join collate thread")
    t_collate.join()
