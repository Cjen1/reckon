...
def setup(hosts, cgrps, logs, **kwargs):
    cluster_caps = ",".join(
        str(i + 1) + ":/data/cap_" + tag(host) for i, host in enumerate(hosts)
    )
    ...
    def run(cmd, tag, host):
        cmd = 'screen -dmS {tag} bash -c "{command} 2>&1 | tee -a {logs}_{tag}"'.format(
            tag=tag, command=cmd, logs=logs
        )
